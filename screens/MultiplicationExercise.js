...
class MultiplicationExercise extends React.Component {
  ...
  newProblem = () => { // PASS IN DIFFICULTIES/N_DIGIT IN THE PROP
    this.setState({ n_1: Math.floor(Math.random() * 10) + 1 })
    this.setState({ n_2: Math.floor(Math.random() * 10) + 1 })

    let n_1_digits = this.props.route.params.n_1_digits;
    let n_2_digits = this.props.route.params.n_2_digits;

    let n_1 = Math.floor(Math.random() * (
      Math.pow(10, n_1_digits) - Math.pow(10, n_1_digits - 1)
    ) + Math.pow(10, n_1_digits - 1))

    let n_2 = Math.floor(Math.random() * (
      Math.pow(10, n_2_digits) - Math.pow(10, n_2_digits - 1)
    ) + Math.pow(10, n_2_digits - 1))
